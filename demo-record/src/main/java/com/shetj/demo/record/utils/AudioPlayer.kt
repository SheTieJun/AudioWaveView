...
sealed class PlaySate {
    object Pause : PlaySate()
    object Stop : PlaySate()
    object Playing : PlaySate()
}
...
class AudioPlayer : MediaPlayer.OnPreparedListener, MediaPlayer.OnErrorListener,
    ...
    var playState = MutableStateFlow<PlaySate>(Stop)

    var playPositionFlow = MutableSharedFlow<Int>(1)
            ...
            when (msg.what) {
                HANDLER_PLAYING -> {
                    if (listener != null
                        ...
                        playPositionFlow.tryEmit(mediaPlayer!!.currentPosition)
                        listener!!.onProgress(mediaPlayer!!.currentPosition, mediaPlayer!!.duration)
                        this.sendEmptyMessageDelayed(HANDLER_PLAYING, 30)
                    }
                }
                HANDLER_START -> {
                    playState.tryEmit(Playing)
     ...
     * @param listener 监听变化
     */
    fun playOrPause(url: String?, listener: PlayerListener? = null) {
        //判断是否是当前播放的url
    ...
    fun setSeekToPlay(seekToPosition: Int) {
        if (isPause) {
            seekToPlay = seekToPosition.coerceAtLeast(1)
        } else {
            seekTo(seekToPosition.coerceAtLeast(1))
        }
    }

    fun setSeekToPlayFloat(@FloatRange(from = 0.0,to = 1.0) float: Float){
        if (mediaPlayer != null){
            mediaPlayer!!.seekTo((mediaPlayer!!.duration*float).toInt())
        }
    }
    ...
    fun getNoPlayTime(): Long {
        return ((mediaPlayer?.duration ?: 0) - (mediaPlayer?.currentPosition ?: 0)).toLong()
    }
}
